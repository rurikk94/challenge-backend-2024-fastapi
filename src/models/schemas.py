...
class Employee(Base):
    ...
    _device_group: Mapped[Optional["DeviceGroup"]] = relationship(
        "DeviceGroup", uselist=False, passive_deletes=True, cascade="all, delete"
    )
    ...
    _enrollments: Mapped[Optional["Enrollments"]] = relationship(
...
class Enrollments(Base):
    ...
    def is_same_photo(self, punch_photo: bytes) -> bool:
        def porcentaje_true(lista: List[bool]):
            total = len(lista)
            true_count = sum(
                1 for elemento in lista if elemento
            )  # Contar cuántos elementos son True
...
class Devices(Base):
    ...
    _device_group: Mapped["DeviceGroup"] = relationship(
        "DeviceGroup", uselist=False, passive_deletes=True, cascade="all, delete"
    )
...
class Timecard(Base):
    ...
    def punch_in(self) -> List[datetime]:
        return [
            punch.punch_dtm_local
            for punch in self.employee._punchs
            if punch.in_out == True and punch.punch_dtm.date() == self.date
        ]
    ...
    def punch_out(self) -> List[datetime]:
        return [
            punch.punch_dtm_local
            for punch in self.employee._punchs
            if punch.in_out == False and punch.punch_dtm.date() == self.date
        ]
