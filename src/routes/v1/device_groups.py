...
@_device_groups.get(
    "/", tags=["device_groups"], response_model=ResponseModel[DeviceGroupsModel]
)
...
@_device_groups.post(
    "/", tags=["device_groups"], response_model=ResponseModel[DeviceGroupDBModel]
)
async def create_device_group(group: DeviceGroupInputModel, db=Depends(get_db)):
    return await create_group(group, db)


async def asignar_devices(
    group_id: Optional[int], lst_devices: List[Devices], db: Session
):
...
async def asignar_employees(
    group_id: Optional[int], lst_employees: List[Employee], db: Session
):
...
@_device_groups.get(
    "/{device_group_id}",
    tags=["device_groups"],
    response_model=ResponseModel[DeviceGroupDBModel],
)
