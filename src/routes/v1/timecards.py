...
@_timecards.get("/{employee_id}", tags=["timecards"], response_model=ResponseModel[TimecardModel])
...
async def get_timecard_by_employee_id(employee_id: int, start: date, end: date, db):
    timecard: TimecardModel = get_all(db, Timecard, employee_id=employee_id, between=[Timecard.date, start, end])
